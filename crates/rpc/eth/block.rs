    ...
    fn handle(&self, storage: Store) -> Result<Value, RpcErr> {
        ...
        match storage.get_latest_block_number() {
            Ok(Some(block_number)) => {
                ...
                let blob_base_fee = calculate_base_fee_per_blob_gas(&parent_header.excess_blob_gas.unwrap_or_default());
...
pub fn get_all_block_rpc_receipts(
    ...
    let blob_gas_price = calculate_base_fee_per_blob_gas(&parent_header.excess_blob_gas.unwrap_or_default());
