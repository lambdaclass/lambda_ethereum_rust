...
static GLOBAL_NONCE_COUNT: AtomicU64 = AtomicU64::new(0);
...
impl Proposer {
    ...
    pub async fn start(&self, head_block_hash: H256, store: Store) -> Result<(), ProposerError> {
        ...
        loop {
            // Setting the nonce in the Atomic variable.
            let nonce = self.eth_client.get_nonce(self.l1_address).await?;
            GLOBAL_NONCE_COUNT.store(nonce, std::sync::atomic::Ordering::Relaxed);
    ...
    pub async fn send_commitment(
        ...
        let mut tx = EIP4844Transaction {
            ...
            nonce: GLOBAL_NONCE_COUNT.fetch_add(1, std::sync::atomic::Ordering::AcqRel),
