use crate::utils::{
    config::{engine_api::EngineApiConfig, eth::EthConfig, xxx::XXXConfig},
...
pub async fn start_xxx(head_block_hash: H256, store: Store) {
    info!("Starting XXX task");
    let eth_config = EthConfig::from_env().unwrap();
    let xxx_config = XXXConfig::from_env().unwrap();
    let engine_config = EngineApiConfig::from_env().unwrap();
    let xxx = XXX::new_from_config(&xxx_config, eth_config, engine_config);
    xxx.start(head_block_hash, store).await;
}
