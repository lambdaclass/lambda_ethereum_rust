...
fn and_basic() {
    ...
    assert_eq!(vm.consumed_gas, TX_BASE_COST + 9);
}
...
fn keccak256_zero_offset_size_four() {
    ...
    assert_eq!(vm.consumed_gas, TX_BASE_COST + 52);
}
...
fn keccak256_zero_offset_size_bigger_than_actual_memory() {
    ...
    assert_eq!(vm.current_call_frame_mut().pc(), 40);
    assert_eq!(vm.consumed_gas, TX_BASE_COST + 61);
}
...
fn keccak256_zero_offset_zero_size() {
    ...
    assert_eq!(vm.consumed_gas, TX_BASE_COST + 34);
}
...
fn keccak256_offset_four_size_four() {
    ...
    assert_eq!(vm.consumed_gas, TX_BASE_COST + 53);
}
...
fn mstore() {
    ...
    assert_eq!(vm.consumed_gas, TX_BASE_COST + 14);
}
...
fn mstore8() {
    ...
    assert_eq!(vm.consumed_gas, TX_BASE_COST + 12);
}
...
fn mcopy() {
    ...
    assert_eq!(vm.consumed_gas, TX_BASE_COST + 35);
}
...
fn mstore_mload_offset_not_multiple_of_32() {
    ...
    assert_eq!(vm.consumed_gas, TX_BASE_COST + 217);
}
...
fn pc_op_with_push_offset() {
    ...
    assert_eq!(vm.consumed_gas, TX_BASE_COST + 5);
}
...
fn blockhash_op() {
    ...
    vm.env.block_env.number = U256::from(current_block_number);
...
fn coinbase_op() {
    ...
    let mut vm = new_vm_with_ops(&operations);
    vm.env.block_env.coinbase = Address::from_low_u64_be(coinbase_address);
...
fn timestamp_op() {
    ...
    let mut vm = new_vm_with_ops(&operations);
    vm.env.block_env.timestamp = timestamp;
...
fn number_op() {
    ...
    let mut vm = new_vm_with_ops(&operations);
    vm.env.block_env.number = block_number;
...
fn prevrandao_op() {
    ...
    let mut vm = new_vm_with_ops(&operations);
    vm.env.block_env.prev_randao = Some(prevrandao);
...
fn gaslimit_op() {
    ...
    let mut vm = new_vm_with_ops(&operations);
    vm.env.block_env.gas_limit = gas_limit;
...
fn chain_id_op() {
    ...
    let mut vm = new_vm_with_ops(&operations);
    vm.env.block_env.chain_id = chain_id;
...
fn basefee_op() {
    ...
    let mut vm = new_vm_with_ops(&operations);
    vm.env.block_env.base_fee_per_gas = base_fee_per_gas;
...
fn blobbasefee_op() {
    ...
    let mut vm = new_vm_with_ops(&operations);
    vm.env.block_env.excess_blob_gas = Some(TARGET_BLOB_GAS_PER_BLOCK * 8);
    vm.env.block_env.blob_gas_used = Some(0);
...
fn blobbasefee_minimum_cost() {
    ...
    let mut vm = new_vm_with_ops(&operations);
    vm.env.block_env.excess_blob_gas = Some(0);
    vm.env.block_env.blob_gas_used = Some(0);
...
fn pop_op() {
    ...
    assert_eq!(vm.consumed_gas, TX_BASE_COST + 8);
}
...
fn jump_op() {
    ...
    assert_eq!(vm.consumed_gas, TX_BASE_COST + 15);
}
...
fn caller_op() {
    ...
    let mut vm = VM::new(Env { tx_env, block_env }, db);
    ...
    assert_eq!(vm.consumed_gas, TX_BASE_COST + gas_cost::CALLER);
}
...
fn origin_op() {
    ...
    assert_eq!(vm.consumed_gas, TX_BASE_COST + gas_cost::ORIGIN);
}
...
fn address_op() {
    ...
    assert_eq!(vm.consumed_gas, TX_BASE_COST + gas_cost::ADDRESS);
}
...
fn selfbalance_op() {
    ...
    assert_eq!(vm.consumed_gas, TX_BASE_COST + gas_cost::SELFBALANCE);
}
...
fn callvalue_op() {
    ...
    assert_eq!(vm.consumed_gas, TX_BASE_COST + gas_cost::CALLVALUE);
}
...
fn codesize_op() {
    ...
    assert_eq!(vm.consumed_gas, TX_BASE_COST + gas_cost::CODESIZE);
}
...
fn gasprice_op() {
    ...
    let tx_env = TxEnv {
        transact_to: TransactTo::Call(address_that_has_the_code),
        effective_gas_price: U256::from(0x9876),
        ..Default::default()
    };
    ...
    assert_eq!(vm.consumed_gas, TX_BASE_COST + gas_cost::GASPRICE);
}
...
fn codecopy_op() {
    ...
    assert_eq!(vm.consumed_gas, TX_BASE_COST + 9 + 3 * gas_cost::PUSHN);
}
...
fn extcodesize_existing_account() {
    ...
    assert_eq!(vm.consumed_gas, 23603);
}
...
fn extcodecopy_existing_account() {
    ...
    assert_eq!(vm.consumed_gas, 23616);
}
