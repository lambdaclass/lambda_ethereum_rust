...
impl VM {
    ...
    pub fn op_add(&mut self, current_call_frame: &mut CallFrame) -> Result<OpcodeSuccess, VMError> {
        if self.consumed_gas + gas_cost::ADD > self.gas_limit {
        ...
        self.consumed_gas += gas_cost::ADD;
    ...
    pub fn op_sub(&mut self, current_call_frame: &mut CallFrame) -> Result<OpcodeSuccess, VMError> {
        if self.consumed_gas + gas_cost::SUB > self.gas_limit {
            return Err(VMError::OutOfGas);
        }
        ...
        self.consumed_gas += gas_cost::SUB;
    ...
    pub fn op_mul(&mut self, current_call_frame: &mut CallFrame) -> Result<OpcodeSuccess, VMError> {
        if self.consumed_gas + gas_cost::MUL > self.gas_limit {
            return Err(VMError::OutOfGas);
        }
        ...
        self.consumed_gas += gas_cost::MUL;
    ...
    pub fn op_div(&mut self, current_call_frame: &mut CallFrame) -> Result<OpcodeSuccess, VMError> {
        if self.consumed_gas + gas_cost::DIV > self.gas_limit {
            return Err(VMError::OutOfGas);
        }
        ...
        self.consumed_gas += gas_cost::DIV;
    ...
    pub fn op_sdiv(
        ...
        if self.consumed_gas + gas_cost::SDIV > self.gas_limit {
            return Err(VMError::OutOfGas);
        }
        let dividend = current_call_frame.stack.pop()?;
        let divisor = current_call_frame.stack.pop()?;
        ...
        current_call_frame.stack.push(quotient)?;
        self.consumed_gas += gas_cost::SDIV;
    ...
    pub fn op_mod(&mut self, current_call_frame: &mut CallFrame) -> Result<OpcodeSuccess, VMError> {
        if self.consumed_gas + gas_cost::MOD > self.gas_limit {
            return Err(VMError::OutOfGas);
        }
        let dividend = current_call_frame.stack.pop()?;
        let divisor = current_call_frame.stack.pop()?;
        ...
        self.consumed_gas += gas_cost::MOD;
    ...
    pub fn op_smod(
        ...
        if self.consumed_gas + gas_cost::SMOD > self.gas_limit {
            return Err(VMError::OutOfGas);
        }
        ...
        self.consumed_gas += gas_cost::SMOD;
    ...
    pub fn op_addmod(
        ...
        if self.consumed_gas + gas_cost::ADDMOD > self.gas_limit {
            return Err(VMError::OutOfGas);
        }
        let augend = current_call_frame.stack.pop()?;
        let addend = current_call_frame.stack.pop()?;
        ...
        current_call_frame.stack.push(remainder)?;
        self.consumed_gas += gas_cost::ADDMOD;
    ...
    pub fn op_mulmod(
        ...
        if self.consumed_gas + gas_cost::MULMOD > self.gas_limit {
            return Err(VMError::OutOfGas);
        }
        ...
        current_call_frame.stack.push(remainder)?;
        self.consumed_gas += gas_cost::MULMOD;
    ...
    pub fn op_signextend(
        ...
        if self.consumed_gas + gas_cost::SIGNEXTEND > self.gas_limit {
            return Err(VMError::OutOfGas);
        }
        ...
        self.consumed_gas += gas_cost::SIGNEXTEND;
