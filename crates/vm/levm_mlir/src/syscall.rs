...
use crate::{
    ...
    program::Program,
    report::{TransactionReport, TxResult, VmError},
    state::AccountStatus,
...
pub struct SyscallContext<'c> {
    ...
    pub halt_reason: Option<VmError>,
    initial_gas: u64,
...
impl<'c> SyscallContext<'c> {
    ...
    pub fn logs(&self) -> Vec<Log> {
        self.inner_context
            ...
            .iter()
            .map(|logdata| Log {
                address: self.env.get_tx_code_address(),
                data: logdata.clone(),
            })
            .collect()
    }

    pub fn get_report(&self) -> TransactionReport {
        ...
        let halt_reason = self.halt_reason.clone().unwrap_or(VmError::UnknownError);

        // TODO: Check if this is ok
        let state = self.journal.into_state();

        let report = match exit_status {
            ExitStatusCode::Return => TransactionReport {
                gas_used,
                gas_refunded,
            ...
            ExitStatusCode::Stop => TransactionReport {
                gas_used,
                gas_refunded,
                output: return_values.into(),
                logs: self.logs(),
            ...
            ExitStatusCode::Revert => TransactionReport {
                output: return_values.into(),
                gas_used,
...
impl<'c> SyscallContext<'c> {
    ...
    pub extern "C" fn call(
        ...
        if off + size > self.inner_context.memory.len() {
            ...
            self.halt_reason = Some(VmError::MemoryOverflow);
        ...
        *consumed_gas = gas_cost::CALL_WARM as u64;
        ...
        let (return_code, return_data) = if is_precompile(callee_address) {
            ...
            let caller_address = self.env.get_tx_code_address();
            ...
            if !value.is_zero() {
                ...
                *consumed_gas += call_opcode::NOT_ZERO_VALUE_COST;
                ...
                if available_gas < *consumed_gas {
                    self.halt_reason = Some(VmError::OutOfGas);
            ...
            *consumed_gas += gas_to_send;
            ...
            let this_address = self.env.get_tx_code_address();
            let (new_frame_caller, new_value, transact_to) = match call_type {
                ...
                CallType::DelegateCall => (self.call_frame.caller, self.env.tx_value, this_address),
            };

            env.tx_value = new_value;
            env.tx_to = Some(transact_to);
            env.tx_gas_limit = gas_to_send;

            //Copy the calldata from memory
            let off = args_offset as usize;
            let size = args_size as usize;
            env.tx_calldata = Bytes::from(self.inner_context.memory[off..off + size].to_vec());
            ...
            executor.execute(&mut context, env.tx_gas_limit);

            let result = context.get_report();
    ...
    pub extern "C" fn store_in_selfbalance_ptr(&mut self, balance: &mut U256) {
        let account = match self.env.tx_to {
            Some(address) => self.journal.get_account(&address).unwrap_or_default(),
            None => AccountInfo::default(), //This branch should never happen
        };
    ...
    pub extern "C" fn read_storage(&mut self, stg_key: &U256, stg_value: &mut U256) -> i64 {
        let address = self.env.get_tx_code_address();
    ...
    pub extern "C" fn write_storage(&mut self, stg_key: &U256, stg_value: &mut U256) -> i64 {
        ...
        let Some(address) = self.env.tx_to else {
            return 0;
        };
    ...
    pub extern "C" fn get_block_hash(&mut self, number: &mut U256) {
        ...
        let hash = if number_as_u256 < self.env.block_number.saturating_sub(EU256::from(256))
            || number_as_u256 >= self.env.block_number
        {
    ...
    pub extern "C" fn get_blob_hash_at_index(&mut self, index: &U256, blobhash: &mut U256) {
        ...
        *blobhash = usize::try_from(index.lo)
            .ok()
            .and_then(|idx| self.env.tx_blob_hashes.get(idx).cloned())
    ...
    fn create_aux(
        ...
        let sender_address = self.env.get_tx_code_address();

        if size > MAX_CODE_SIZE * 2 {
            self.halt_reason = Some(VmError::ContractSizeLimit);
            return return_codes::HALT_RETURN_CODE;
        }
        ...
        let (dest_addr, hash_cost) = match salt {
            ...
            _ => {
                if sender_account.nonce.checked_add(1).is_none() {
                    self.halt_reason = Some(VmError::NonceOverflow);
                    return return_codes::HALT_RETURN_CODE;
                }
        ...
        if self.journal.get_account(&dest_addr).is_some() {
            self.halt_reason = Some(VmError::ContractAlreadyExists);
            return return_codes::HALT_RETURN_CODE;
        }
        ...
        new_env.tx_to = Some(dest_addr);
        new_env.tx_gas_limit = *remaining_gas;
        ...
        executor.execute(&mut context, new_env.tx_gas_limit);

        let result = context.get_report();
        let bytecode = result.output;
        ...
        let gas_cost =
            init_code_cost + code_deposit_cost + hash_cost + result.gas_used - result.gas_refunded;
...
pub mod symbols {
    // Global variables
    pub const CONTEXT_IS_STATIC: &str = "levm_mlir__context_is_static";
    // Syscalls
    pub const WRITE_RESULT: &str = "levm_mlir__write_result";
    pub const EXTEND_MEMORY: &str = "levm_mlir__extend_memory";
    pub const KECCAK256_HASHER: &str = "levm_mlir__keccak256_hasher";
    pub const STORAGE_WRITE: &str = "levm_mlir__write_storage";
    pub const STORAGE_READ: &str = "levm_mlir__read_storage";
    pub const APPEND_LOG: &str = "levm_mlir__append_log";
    pub const APPEND_LOG_ONE_TOPIC: &str = "levm_mlir__append_log_with_one_topic";
    pub const APPEND_LOG_TWO_TOPICS: &str = "levm_mlir__append_log_with_two_topics";
    pub const APPEND_LOG_THREE_TOPICS: &str = "levm_mlir__append_log_with_three_topics";
    pub const APPEND_LOG_FOUR_TOPICS: &str = "levm_mlir__append_log_with_four_topics";
    pub const GET_CALLDATA_PTR: &str = "levm_mlir__get_calldata_ptr";
    pub const GET_CALLDATA_SIZE: &str = "levm_mlir__get_calldata_size";
    pub const GET_CODESIZE_FROM_ADDRESS: &str = "levm_mlir__get_codesize_from_address";
    pub const COPY_CODE_TO_MEMORY: &str = "levm_mlir__copy_code_to_memory";
    pub const GET_ADDRESS_PTR: &str = "levm_mlir__get_address_ptr";
    pub const GET_GASLIMIT: &str = "levm_mlir__get_gaslimit";
    pub const STORE_IN_CALLVALUE_PTR: &str = "levm_mlir__store_in_callvalue_ptr";
    pub const STORE_IN_BLOBBASEFEE_PTR: &str = "levm_mlir__store_in_blobbasefee_ptr";
    pub const GET_BLOB_HASH_AT_INDEX: &str = "levm_mlir__get_blob_hash_at_index";
    pub const STORE_IN_BALANCE: &str = "levm_mlir__store_in_balance";
    pub const GET_COINBASE_PTR: &str = "levm_mlir__get_coinbase_ptr";
    pub const STORE_IN_TIMESTAMP_PTR: &str = "levm_mlir__store_in_timestamp_ptr";
    pub const STORE_IN_BASEFEE_PTR: &str = "levm_mlir__store_in_basefee_ptr";
    pub const STORE_IN_CALLER_PTR: &str = "levm_mlir__store_in_caller_ptr";
    pub const GET_ORIGIN: &str = "levm_mlir__get_origin";
    pub const GET_CHAINID: &str = "levm_mlir__get_chainid";
    pub const STORE_IN_GASPRICE_PTR: &str = "levm_mlir__store_in_gasprice_ptr";
    pub const GET_BLOCK_NUMBER: &str = "levm_mlir__get_block_number";
    pub const STORE_IN_SELFBALANCE_PTR: &str = "levm_mlir__store_in_selfbalance_ptr";
    pub const COPY_EXT_CODE_TO_MEMORY: &str = "levm_mlir__copy_ext_code_to_memory";
    pub const GET_PREVRANDAO: &str = "levm_mlir__get_prevrandao";
    pub const GET_BLOCK_HASH: &str = "levm_mlir__get_block_hash";
    pub const GET_CODE_HASH: &str = "levm_mlir__get_code_hash";
    pub const CALL: &str = "levm_mlir__call";
    pub const CREATE: &str = "levm_mlir__create";
    pub const CREATE2: &str = "levm_mlir__create2";
    pub const GET_RETURN_DATA_SIZE: &str = "levm_mlir__get_return_data_size";
    pub const COPY_RETURN_DATA_INTO_MEMORY: &str = "levm_mlir__copy_return_data_into_memory";
    pub const TRANSIENT_STORAGE_READ: &str = "levm_mlir__transient_storage_read";
    pub const TRANSIENT_STORAGE_WRITE: &str = "levm_mlir__transient_storage_write";
    pub const SELFDESTRUCT: &str = "levm_mlir__selfdestruct";
}
