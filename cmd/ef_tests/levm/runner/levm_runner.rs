...
pub fn prepare_vm_for_tx(vector: &TestVector, test: &EFTest) -> Result<VM, EFTestRunnerError> {
    ...
    VM::new(
        ...
        Environment {
            ...
            tx_max_fee_per_gas: test.transactions.get(vector).unwrap().max_fee_per_gas,
            tx_max_fee_per_blob_gas: test.transactions.get(vector).unwrap().max_fee_per_blob_gas,
        },
